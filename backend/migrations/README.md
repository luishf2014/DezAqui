...
Há **37** ficheiros, numerados de **`001_*.sql`** a **`037_*.sql`**. A ordem é **só a ordem do número** no nome (001 … 037).
...
2. Correr o conteúdo de `001_init.sql`, depois `002_…`, e assim sucessivamente até `037_…`.

Não modifique uma migração já aplicada em produção: crie uma nova (por exemplo `038_…`).
